...
function user(inthn,gender,month,year,group){
	var db = openDatabase('hospital', '1.0', 'hospitaldb', 2 * 1024 * 1024);
	
	db.transaction(function(tx) {
		tx.executeSql('CREATE TABLE IF NOT EXISTS user (hn string primary key, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, gender string, month integer, year integer, gp string, eva integer)');

		tx.executeSql("INSERT INTO user (hn,gender,month,year,gp,eva) VALUES (?,?,?,?,?,?)", [inthn,gender,month,year,group,0], function(tx, res) {
...
function con(inthn,group){
	...
	db.transaction(function(tx) {
		if(group == "A"){
			tx.executeSql('DELETE FROM groupA WHERE id IN (SELECT id FROM groupA ORDER BY id DESC LIMIT 1)',[], function(tx, res) {
			...
			tx.executeSql('DELETE FROM groupB WHERE id IN (SELECT id FROM groupB ORDER BY id DESC LIMIT 1)',[], function(tx, res) {
